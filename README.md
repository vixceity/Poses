...
3. Player 2 copies the three poses **in order**, holding each for two seconds within the tolerance. The target skeleton and matrix appear beside the camera. Each target has a fresh 20-second deadline by default. Wrong poses can be retried until that deadline.
...
| --- | --- | --- |
| `CAMERA_INDEX` | `0` | OpenCV camera device number |
